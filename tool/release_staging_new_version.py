...
from tools import STATE_INITIAL, STATE_VERSION_PUBSPEC_UPDATED, STATE_VERSION_CHANGELOG_UPDATED, STATE_RELEASE_BRANCH_CREATED, STATE_CHANGES_STAGED, STATE_CHANGES_COMMITTED, STATE_CHANGES_PUSHED, STATE_PR_DEV_CREATED, STATE_PR_PROD_CREATED, STATE_TAG_CREATED, STATE_TAG_PUSHED, STATE_PR_DEV_MERGED, run_step, checkout_development_branch, merge_pr_bitbucket, create_tag, push_tag, create_pull_request, push_changes, commit_changes, create_release_branch, update_version_changelog, update_version_pubspec, is_version_app_valid, is_current_branch_dev, is_development_branch_up_to_date, update_version_changelog_project, stage_changes_app, revert_process_app
import json
import os.path
...
try:
    # 1. Check whether your current branch is development branch.
    run_step("Checking whether your current branch is development branch...", is_current_branch_dev, DEVELOPMENT_BRANCH)

    # 2. Check whether your development branch is up to date.
    ...
    while is_version_app_valid(previous_version) != True:
        previous_version = input('🔵 What is the current version? ')
        if is_version_app_valid(previous_version) != True:
            print("     ⛔️ ERROR: The current version must use convention: X.Y.Z+A . For example: 1.2.3+4")
        else:
            print("     ✅ OK")

    # Prompts user for the new version.
    while is_version_app_valid(new_version) != True:
        new_version = input('🔵 What is the new version? ')
        if is_version_app_valid(new_version) != True:
            print("     ⛔️ ERROR: The new version must use convention: X.Y.Z+A . For example: 1.2.3+4")
        else:
            print("     ✅ OK")
    ...
    # Update version in CHANGELOG_PROJECT.md if exists
    run_step("   Updating version in CHANGELOG_PROJECT.md...", update_version_changelog_project, new_version, PROJECT_NAME, DEVELOPMENT_BRANCH, previous_version)
    ...
    # Stage the changes
    run_step("4. Staging changes...", stage_changes_app)
    ...
    _, __, pr_id = run_step(
        "8. Creating Pull Request to staging branch...",
        create_pull_request,
        PROJECT_NAME, 
        ACCESS_TOKEN, 
    ...
    print("   > PR staging branch: {}".format("https://bitbucket.org/elevarm/{}/pull-requests/{}".format(PROJECT_NAME, pr_stg_id)))
    print("")
    ...
    # Merge PR to staging
    run_step(
        "12. Merging Pull Request to staging branch...",
        merge_pr_bitbucket,
        PROJECT_NAME,
        ACCESS_TOKEN,
        pr_stg_id,
    ...
    revert_process_app(state, DEVELOPMENT_BRANCH, release_branch, new_version)
    exit(1)
